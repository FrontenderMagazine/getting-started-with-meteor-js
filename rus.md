Ниже приведен гостевой пост от [Бена Страхана][1] — члена клуба Meteor.js. Он написал крутой обобщающий пост, и я хочу им поделится со всем клубом!
...
Последующие списки специально даны в определённом порядке, если не сказано иного. Эта статья не объясняет почему вы должны изучать то или это (вы сами должны в этом разобраться). Вместо этого цель статьи — это дать быструю карту или вид «тысячной мили» технологий, с которыми ежедневно имеет дело Метеор разработчик. 

Когда вы пробираетесь через тернии, получая новые знания, то хорошо бы иметь под рукой карту, на которую можно взглянуть и узнать свой прогресс.

![Побег Немо][3]

## Языки, библиотеки и фреймворки, трындец!

В конечном счёте вам нужно уметь понимать [Meteor API][4]. Изучение технологий, приведенных ниже, даст вам то, что нужно. Нет нужды становится экспертом в каждой области, но вам нужно понимать структуру и терминологию каждой. 
...
Теперь, когда вы знаете приведенное выше, вы достойны чести познать силу и великолепие Meteor!

![газонокосилка работающая на meteor][11]

Почему вам нужно изучить ВСЁ приведенное выше, перед тем как прикасаться к Meteor? Потому что Meteor — это [Full-Stack][12] платформа. Через Meteor вы управляете фронт-эндом, бек-эндом и всеми остальными эндами.
...
Ух ты, вы должно быть упорные, раз дошли до этого места. Ладно, вы хотите знать мои суперсекретные списки?
...
### Блоги, блоги, новости и остальное (в произвольном порядке) 
...
Если я забыл кого-то, то дайте мне знать на ([@_benstr][1]) или [@joshowens][22] 
