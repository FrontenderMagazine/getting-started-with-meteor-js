Ниже приведен гостевой пост от [Бена Страхана][1] — члена клуба Meteor.js. Он 
...
Что должен знать вдохновлённый веб-разработчик, чтобы создавать приложения 
на [Meteor][2]?
Ниже приведен список языков, фреймворков, библиотек, пакетов и многое другое ;)

Нижеследующие списки специально даны в определённом порядке, если не сказано 
иное. Эта статья не объясняет, почему вы должны изучать ту или иную 
техноголию (вы сами должны в этом разобраться). Цель статьи — дать быстрый
обзор технологий, с которыми ежедневно имеет дело Meteor-разработчик. 

Когда вы пробираетесь через тернии, получая новые знания, хорошо бы иметь под 
рукой карту, на которую можно взглянуть, чтобы узнать в какой точке
пути вы находитесь.
...
приведенных ниже, даст вам необходимое. Нет нужды становится экспертом в 
каждой области, но вам нужно понимать структуру и терминологию каждой из них. 

Вы не знаете, что такое API? [Посмотрите на видео этого парня][5]
...
Теперь, когда вы изучили приведенное выше, вы достойны чести познать 
силу и великолепие Meteor!

![газонокосилка работающая на meteor][11]

Почему вам нужно изучить ВСЁ содержимое предыдущего списка, перед тем как
...
Пришло время становится Meteor-задротом, читайте доки.

* [Основная документация по API][4]
* [Подпроекты][13]

Если подпроекты выглядят пугающе, не волнуйтесь. Ниже приведен 
минимум ключевых пакетов из подпроектов, необходимых для изучения.
...
проведут вас по различным проектам Meteor. Это будет кульминация 
всего, приведенного выше.
...
Да, есть ещё много всего для изучения. У Метеора есть менеджер пакетов,
который называется [Atmosphere][18]. Он позволяет сообществу создавать 
пакеты, которые глубоко интегрированы в платформу Meteor и расширяют 
доступные для вас API. Ниже приведен список из стандартных пакетов, 
которые вы найдёте в почти каждом серьёзном Meteor-приложении, поэтому 
их нужно знать.
...
Ух ты, вы должно быть упорные, раз дошли до этого места. Ладно, вы 
хотите увидеть мои суперсекретные списки?
...
которые служат целям Meteor-сообщества (и делают отличную работу), поэтому я 
решил озвучить их имена.
